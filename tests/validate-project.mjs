...
assert.equal(GAME_VERSION, '0.1.3.2.1.2', 'Hotfix version must be v0.1.3.2.1.2');
assert.ok(html.includes('v0.1.3.2.1.2'), 'Build shell must identify v0.1.3.2.1.2');
...
assert.ok(!worldSource.includes('releaseAssetsNotNeededForMap('), 'v0.1.3.2.1.2 must not eagerly evict textures during the WebKit-sensitive Scene handoff');
...
console.log(`Validated ${ASSET_DEFS.length} assets, ${Object.keys(ITEM_DEFS).length} items, ${Object.keys(ENEMY_DEFS).length} enemies, ${Object.keys(NPC_DEFS).length} NPCs, ${BUILDING_DEFS.length} refuge buildings, ${COLLIDERS.length} visible-source colliders, ${Object.keys(QUEST_DEFS).length} quests, ${Object.keys(SKILL_DEFS).length} player skills, ${Object.keys(ENEMY_ABILITY_DEFS).length} enemy abilities, v0.1.3.2.1.2 final combat-HUD tightening, staged Azrael/Assassin source sheets, v0.1.3.2 recovery/consumables, stackable supplies, sanctuary/merchant recovery, v0.1.3.1 combat polish, true run/spear thrust, skill-rank hooks, expanded LPC actions, pooled projectiles, statuses/FX/audio, stable Cinder/Hollow streaming, combo-safe starter clothes, and save schema ${SAVE_VERSION}.`);
