...
assert.equal(GAME_VERSION, '0.1.3.2.3', 'Celestial-expansion version must be v0.1.3.2.3');
assert.ok(html.includes('v0.1.3.2.3'), 'Build shell must identify v0.1.3.2.3');
...
assert.ok(!worldSource.includes('releaseAssetsNotNeededForMap('), 'v0.1.3.2.3 must not eagerly evict textures during the WebKit-sensitive Scene handoff');
...
assert.equal(AZRAEL_DEF.internalLevel, 99, 'Azrael field-test stats must be endgame-scale while the visible level remains hidden');
...
assert.ok(azraelSource.includes('majorAbilityLockUntil') && azraelSource.includes('hostileCountNear') && azraelSource.includes('azrael_sanctified_nova') && azraelSource.includes('azrael_seraphic_judgment'), 'Azrael AI must pace and select the expanded six-skill kit');
...
assert.ok(fxManagerSource.includes('celestialSigil') && fxManagerSource.includes('celestialWingBurst') && fxManagerSource.includes('ancientCelestialSeal') && fxManagerSource.includes('sanctifiedNovaImpact') && fxManagerSource.includes('seraphicJudgmentSeal') && fxManagerSource.includes('seraphicJudgmentImpact') && fxManagerSource.includes('heavenfallImpact'), 'Azrael attacks must use a distinct ancient-celestial VFX language');
assert.ok(combatSource.includes('azrael_sanctified_nova') && combatSource.includes('azrael_seraphic_judgment') && combatSource.includes('pulseScales'), 'CombatSystem must execute both new AoE skills including Seraphic Judgment multi-hit pulses');
assert.ok(audioManagerSource.includes('sanctified_nova') && audioManagerSource.includes('seraphic_judgment'), 'Expanded Azrael skills need distinct procedural audio signatures');
