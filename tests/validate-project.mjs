...
assert.equal(GAME_VERSION, '0.1.3.2.2', 'Field-test version must be v0.1.3.2.2');
assert.ok(html.includes('v0.1.3.2.2'), 'Build shell must identify v0.1.3.2.2');
...
assert.ok(!worldSource.includes('releaseAssetsNotNeededForMap('), 'v0.1.3.2.2 must not eagerly evict textures during the WebKit-sensitive Scene handoff');
...
assert.ok(fxManagerSource.includes('celestialSigil') && fxManagerSource.includes('celestialWingBurst') && fxManagerSource.includes('heavenfallImpact'), 'Azrael attacks must use a distinct celestial VFX language');
