...
assert.equal(GAME_VERSION, '0.1.4.4.3.1', 'Lailani Field-Test Polish version must be v0.1.4.4.3.1');
assert.ok(html.includes('v0.1.4.4.3.1'), 'Build shell must identify v0.1.4.4.3.1');
...
console.log(`Validated ${ASSET_DEFS.length} assets, ${Object.keys(ITEM_DEFS).length} items, ${Object.keys(ENEMY_DEFS).length} enemies, ${Object.keys(NPC_DEFS).length} NPCs, ${BUILDING_DEFS.length} refuge buildings, ${COLLIDERS.length} visible-source colliders, ${Object.keys(QUEST_DEFS).length} quests, ${Object.keys(SKILL_DEFS).length} player skills, ${Object.keys(ENEMY_ABILITY_DEFS).length} enemy abilities, v0.1.4.4.3.1 Lailani Field-Test Polish, eight streamed maps, ten POIs, six world-event seeds, reusable nested return anchors, debug-only First-Light stress warband, bounded faction assist, target-loss recovery, pursuit leashes, preserved common celestial troops and faction-safe shared combat, preserved Demon Combat Foundation and Refuge geometry/building integrity, Living Wilds ecology, grouped aggro, authored patrols, proximity ambushes, layered hostile humans, four-role Demon Legion patrol, bounded offscreen simulation, preserved v0.1.4.1 region scale/refuge rebuild, inherited Sanctuary of the First Light, seven-skill paced Azrael AI, faction combat, knockback/shake, contribution-gated rewards, recovery/consumables, stable map streaming, and save schema ${SAVE_VERSION}.`);
